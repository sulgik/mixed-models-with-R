...
아래애서 우리는 이러한 데이터 상황들에서 mixed effect model 을 살펴볼 것이다. 일반적으로 우리는 클러스터링이 모델보다는 데이터의 특성이라는 접근법을 취할 것이다. 그러나, mixed model 은 유연해서 다양한 데이터 상황을 다룰 수 있다는 것을 이해하는 것이 중요하다. 
...
기본모형을 여러 방법으로 설명할 수 있다. 첫번째로는 표준 회귀부터 시작해서 방향을 잡아본다.
...
절편과 시간효과 계수 ($b$) 가 있다. 오차 ($\epsilon$) 는 평균 0 이고 어떤 편차 $\sigma$ 를 가지는 정규분포를 이룬다고 가장한다 .
...
또 다른 방법인, $\mathrm{gpa}$ 데이터를 생성하는 과정을 강조하는 모델은 다음과 같이 쓸 수 있다. 
...
따라서 학생효과는 랜덤하고 구체적으로는 평균 0 과 측정된 표준편차 ($\tau$) 를 갖는 정규분포를 이룬다. 다른 말로 하면, mixed model 과 표준 회귀 사이의 개념적 차이는 학생 효과 뿐인데, 평균적으로 효과는 없지만 일반적으로 평균적으로 $\tau$ 값이 학생마다 차이나는 것이다.
...
다시 정리하면, 오차의 추가 소스보다 모델 계수들에 집중할 수 있다.
...
우리는 학생마다 절편을 갖게 되어서, 학생 각자가 전체 절편에 고유 효과가 더해지게 되어, 결국 각 사람마다 다른 절편을 갖게 된다.
...
이제 절편이 평균이 전체 절편이고 어떤 표준편차를 가지는 정규분포를 이루는 것을 보았다. 이를 종종 *랜덤 절편* 모형이라고 부른다.
...
우리는 occasion 마다 학생에 특수한 효과를 갖지 않는다는 것을 주목하라. 이러한 배경에서, 학기는 *fixed effect* 만 갖고, 랜덤 파트는 없다고 말한다. 항상 이렇다고는 할 수 없는데, 나중에 볼 것이다. 
...
모든 학생 path들은 연한 선으로 표현되었고 10개 샘플은 진한색으로 표현되어있다. 나중에 볼 회귀모형으로 측정한 전체 트렌드는 빨간색으로 나타냈다. 두 가지가 눈에 띈다. 하나는 처음에 변동이 큰 학생들이다. 두번째는 일반적으로 GPA 트렌드가 상향하지만, 개별 학생들은 궤도에 있어 차이를 보인다는 점이다.
...
시작해보자. 우선, 회귀를 살펴보는데, 시간 indicator 를 covariate 로 두는데 수치형으로 할 것이다. 여기서 나는 깨끗한 버전의 요약된 objects 를 사용할 것이라는 점을 주목하라.
...
`r round(coef_lm[1], 2)`.  In addition, as we move from semester to semester, we can expect GPA to increase by about `r round(coef_lm[2], 2)`
