...
표준 회귀의 결과와 비교하여 새로운 점 중의 하나는 student 효과 의 분산/표준편차 측정값 (앞에서 본 공식 의 $\tau$) 이다. 이는 우리가 학생마다 살펴볼 때 GPA 가 편균적으로 얼마나 바뀌는지를 알려준다. 다른말로 하면, 시점에 기반한 예측을 한 뒤라도, 각 학생들은 자신의 고유한 변동을 가지고 있고, (편차를 통해) 이 값은 학생들 사이의 평균 편차의 측정값이다. 학생에 기반하여 점수가 바뀌는 것은 학기가 바뀌는 것에 기반한 변화량의 두배 이상인 것을 주목하라. 
