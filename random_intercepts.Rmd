...
mixed model 을 표현하는 다른 방법은 흔히 *multilevel 모델링* 논문에서 볼 수 있다. 두 개의 파트 회귀 모형으로 더 명시적으로 표현할 수 있는데, 한 파트는 관측값 레벨과 다른 파트는 학생 레벨이다.
...
시작해보자. 우선, 회귀를 살펴보는데, 시간 indicator 를 covariate 로 두는데 수치형으로 할 것이다. 여기서 나는 요약된 객체들 중 깔끔한 버전을 사용할 것이라는 점을 주목하라.
...
이는 절편이 의미하는 시작시점에서, 즉 학기가 0 일 때 평균 학점이 `r round(coef_lm[1], 2)` 임을 보여준다. 또한, 학기가 진행됨에 따라, GPA 가 약 `r round(coef_lm[2], 2)` 점 만큼 증가함을 알 수 있다. 우리가 군집을 무시한다는 점을 제외하고는 큰 문제가 생기지 않는다. 이렇게 하는 것의 단점은 표준오차가 편향되고 따라서 이를 기반으로 한 통계적 유의성에 관한 주장은 잘못될 것이라는 점이다. 하지만 더 중요한 점은 바로 우리의 관심사인 학생 효과를 탐색할 수 없다는 점이다.
...
다음으로 학생 효과가 있는 mixed model 을 수행한다. 이 모델은 R 에서 <span class="pack">lme4</span> 패키지로 쉽게 수행할 수 있다. 다음의 코드는 <span class="pack">lme4</span> 으로 회귀에 사용한 것과 똑같이 보일 수 있지만, 그룹을 특정하는 추가요소, 즉 학생 효과가 있다. `(1|student)` 는 `1` 로 표시하는 절편이 학생마다 변하도록 허용한다는 것을 의미한다. 이러한 mixed model 로 회귀와 같은 결과를 얻겠지만, 이에 대해서는 뒤에서 이야기 할 것이다.
...
절편과 시간에 관한 계수들, 즉 우리 상황에서는 *fixed* 효과들은 표준 회귀에서 보았던 것과 같고[^lmlmercoef], 해석도 그렇다는 것을 확인할 수 있다. 반면 결국에는 통계적 유의성이 같은 한, 우리의 결론도 같지만, 표준오차는 *여기서 다르다. 절편의 표준오차가 증가했다는 것에 주목하라. 개념적으로 사람마다 랜덤 절편을 허용해서 개인에 대한 정보를 얻게 해 주는 반면, 우리가 전에 과소추정한[^sewithin] 전체 평균에 관해서 불확실성을 알 수 있다고 할 수 있다. 

계수들과 표준 오차들이 있지만, <span class="pack">lme4</span> 은 p-value 를 주지 않는다는 것을 알아차렸는가? 여기에는 [몇몇 이유](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#why-doesnt-lme4-display-denominator-degrees-of-freedomp-values-what-other-options-do-i-have) 가 있는데, mixed model 로 다루는 것이 첫번째로는 클러스터마다 샘플 크기가 다른 것들 $N_c$ 인데, 하나의 관측값이 있는 경우도 있다. 두번째로는  N 개의 총 관측값을 다루는데, 이는 레퍼런스 분포, 분모의 자유도와 최상의 해법을 근사하는 방법에 있어서 상황이 이상해지게 된다. 자동으로 p-value 를 제공하는 다른 프로그램들은 이슈가 없는 것같고, p-value 를 계산하는 여러 방법 중 어떤 방법을 사용했는지 말해주지 않는다. 더군다나, 이러한 근사법은 어떤 시나리오에서는 매우 안 좋거나, 그 상황[^fuzzyp]에 적절하지 않은 가정을 한다. 
...
표준 회귀의 결과와 비교하여 새로운 점 중의 하나는 student 효과 의 분산/표준편차 측정값 (앞에서 본 공식 의 $\tau$) 이다. 이는 우리가 학생마다 살펴볼 때 GPA 가 편균적으로 얼마나 바뀌는지를 알려준다. 다른말로 하면, 시점에 기반한 예측을 한 뒤라도, 각 학생들은 자신의 고유한 변동을 가지고 있고, (편차를 통해) 이 갓ㅂㅅ은 학생들 사이의 평균 편차의 측정값이다. 학생에 기반하여 점수가 바뀌는 것은 학기가 바뀌는 것에 기반한 변화량의 두배 이상인 것을 주목하라. 

분산 출력값을 해석하는 다른 방법은 전체 분산에 비례한 학생 분산의 백분율인데 `r round(vcovs[1,1], 3)` / `r round(sum(vcovs), 3)` =  `r round(vcovs[1,1]/sum(vcovs), 2)*100`% 와 같다. 이를 <span class="emph">intraclass correlation</span> 라고 부르는데, within 클러스터 상관계수의 추정값인데 뒤에 살펴보겠다. 


#### 랜덤효과의 추정값

모델을 실행한 후, 학생 효과[^blup] 의 값을 구할 수 있다. 앞에서 부터 다섯 명의 학생에 대해 두가지 방법에 대해 보여주는데, 하나는 랜덤효과로 다른 하나는 랜덤 절편이다. (즉, 절편 + 랜덤 효과)
...
우리는 학기가 모든 학생들마다 변하지 않고 일정하다고, 즉, *fixed* effect 라고 가정했다. 

우리는 이러한 효과들에 매우 관심이 있고, 이에 관한 불확실성을 알고 싶다. <span class="pack">lme4</span> 로는 bootstrapping 을 통해 일반적으로 할 수 있는데, 구체적으로는 <span class="pack">lme4</span> 안에 있는 <span class="func">bootMer</span> 함수이다. 하지만, 일부 독자에게는 약간 복잡한 작업일 수 있다. <span class="pack">merTools</span> 패키지의 <span class="func">predictInterval</span> 함수[^predinterval]는 쉬운 방법을 제공한다. 바로 이들이 제공하는 플롯으로 가 보자. 
...
다음의 플롯은 각 학생마다의 랜덤 효과 측정값과 이들의 구간 추정값이다 (코드[^mertoolsplotlabels]의 마지막 라인이 만든 플롯의 수정버전). 랜덤 효과는 수평선으로 나타낸, 평균 0 인 정규분표를 이루고 있음을 기억하라. 0을 포함하지 않는 구간은 굵게 나타내었다. 
...
표준 예측 대비 클러스터-기반 예측을 살펴보자. 대부분의 R 모델들과 같이, 모델 오브젝트에 predict 함수를 사용할 수 있다. 
...
학생 효과의 측정값에 따라, 모든 학생들의 절편 측정값의 위 아래 값에서 시작할 것이다. 다음은 앞에서 부터 두명의 학생에 대한 비조건부(unconditional) 예측 vs. 랜덤 절편을 포함한 조건부 예측을 시각화한다.
...
우리는 mixed model 의 예측값은 다른 절편을 갖기 때문에 옮겨졌음을 볼 수 있다. 이러한 학생들의 변동은 상대적으로 안좋게 시작했음을 나타낸다.

## Cluster Level Covariates

Mixed model 을 multilevel 모델로 살펴보자.

$$\mathrm{gpa} = b_{\mathrm{int\_student}} + b_{\mathrm{occ}}\cdot \mathrm{occasion} + \epsilon$$

$$b_{\mathrm{int\_student}} = b_{\mathrm{intercept}} + \mathrm{effect}_{\mathrm{student}}$$
예를 들어 성별과 같은 학생 수준의 covariate 을 모델에 더한다면, 다음과 같이 된다. 

$$b_{\mathrm{int\_student}} = b_{\mathrm{intercept}} + b_{sex}\cdot \mathrm{sex} +  \mathrm{effect}_{\mathrm{student}}$$
집어 넣은 다음 우리는 전과 같은 같은 모델에 설명변수들이 추가된 모델을 갖는다.
...
따라서, 클러스터 레벨 covariate 을 추가한다고 해서, 모델[^mlevel] 에 대해 생각하는 방법에 대해 이상한 효과를 주는 것은 아니다. 우리는 단순히 설명 변수들에 추가하기만 하면 된다. 관측값 레벨 변수들의 평균이다 다른 요약값으로 클러스터 레벨 covariate 을 만들 수도 있다. 클러스터들이 지리적 단위를 대표하고 관측값들이 사람들인 경우에 대해 매우 자주 일어난다. 예를 들어, 소득을 개인레벨 covairate 으로 갖고, 그 지역의 평균 경제적 수준을 대표하기 위해 중간값을 사용할 수 있다.
