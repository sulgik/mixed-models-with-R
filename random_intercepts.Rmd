...
Mixed model 은 오랜동안 통계학 영역에서 사용되어 왔다. 예를들어, 표준적인 ANOVA 방법들은 mixed model 의 특별한 케이스라고 볼 수 있다. 최근에는, mixed model 은 다양하게 응용되었고 확장되어, 다양한 범위의 데이터 상황들에 적용할 수 있게 되었다. 일반화 선형모형 이상의 툴셋을 확장하는데에 첫 걸음으로 볼 수 있다. 
...
For the following we'll demonstrate the simplest and most common case of a mixed model, that in which we have a single grouping/cluster structure for the random effect.  For reasons that will hopefully become clear soon, this is commonly called a *random intercepts model*. 
...
시작해보자. 우선, 회귀를 살펴보는데, 시간 indicator 를 covariate 로 두는데 수치형으로 할 것이다. 여기서 나는 여기에서 요약된 객체들 중 깔끔한 버전을 사용할 것이라는 점을 주목하라.
...
절편과 시간에 관한 계수들, 즉 우리 상황에서는 *fixed* 효과들은 표준 회귀에서 보았던 것과 같고[^lmlmercoef], 해석도 그렇다는 것을 확인할 수 있다. 반면 결국에는 통계적 유의성이 같은 한, 우리의 결론도 같지만, 표준오차는 *여기서 다르다. 절편의 ㅇ표준오차가 증가했다는 것에 주목하라. 개념적으로 
...
모델을 실행한 후, 학생 효과[^blup] 의 값을 구할 수 있다. 앞에서 부터 다섯 학생에 대해 두가지 방법에 대해 보여주는데,하나는 랜덤효과로 다른 하나는 랜덤 절편이다. (즉, 절편 + 랜덤 효과)
...
다음의 플롯은 각 학생마다의 랜덤 효과 측정값과 이들의 구간 추정값이다 (코드[^mertoolsplotlabels]의 마지막 라인이 만든 플롯의 수정버전). 랜덤 효과는 수평선으로 나타낸, 평균 0 인 정규분표를 이루고 있음을 기억하라. 0을 포함하지 않는 구간은 굵은 글씨로 나타내었다. 
...
학생 효과의 측정값에 따라, 모든 학생들의 절편 측정값의 위 아래 값에서 시작할 것이다. 다음은 위에서 부터 두명의 학생에 대한 비조건부(unconditional) 예측 vs. 랜덤 절편을 포함한 조건부 예측을 시각화한다.
